...
import { readConfig, CONFIG_NAME } from './common.mjs';
...
process.stdin.on('end', async () => {
  ...
  let root, config;
  try { ({ root, config } = readConfig(event.cwd || process.cwd())); }
  catch (error) {
  ...
  const context = [
    ...
    'Before answering project-specific questions, making architecture decisions, planning changes, or implementing, use semantic-search docs_search with the actual task and read relevant source sections with docs_read or filesystem tools. Pass the current absolute working directory as cwd.',
    ...
    'Use $semantic-search:search for the retrieval workflow. No search is needed for unrelated conversation or plugin setup itself.',
