...
Opening the dashboard and refreshing status only read the shared queue journal and existing indexes. These actions do not start the indexing daemon or download models.

Select a project, enter a question and press **Найти** (or Enter) to search. The dashboard uses the same semantic and keyword search as `docs_search`, returning up to six fragments with document headings, relative paths and source line ranges. Each file path is clickable and opens the configured document in the operating system’s default application. The default application chooses the editor and initial cursor position; line numbers remain visible in the dashboard. Result cards show current section context with the indexed fragment’s source lines highlighted; the exact indexed fragment is available in a disclosure. Small sections are shown in full. Large sections initially show a bounded window (up to 24,000 characters), with **Показать весь раздел** loading the complete section. If the matching source lines have changed or disappeared, the card retains the indexed text and shows an explicit warning. A text-match label distinguishes literal substrings in the fragment, heading or path from semantic-only candidates; it is not a relevance confidence score.

The dashboard requests up to 20 ranked candidates, removes standalone Markdown headings/separators and merges identical section previews before showing up to six results. This works with existing indexes and already running daemons; it does not change the underlying MCP/CLI ranking or add a relevance threshold. Loading, empty results and search errors are shown separately from indexing status. Submitting a search starts the shared service when needed, temporarily registers the project and waits for known indexing work; first use may download the runtime/model. Later queued changes are reported alongside results. Counts describe the saved index, not a percentage of current source coverage. When the indexing service is offline, persisted work is labeled as interrupted/recovery state. Read and connection errors are shown explicitly.

Its separate server binds only to `127.0.0.1`, uses a private unguessable URL, rejects foreign browser origins/Host headers and requires same-origin JSON POST requests for search and external file opening. File/context endpoints only read configured documentation, reject traversal and symlinks, and open paths as process arguments without shell interpolation. No external assets are served; search and file previews return local document text without executing Markdown/HTML. Keep the local URL private; anyone with it on the same computer can view project names, file names and searchable document contents. The server exits after two minutes without page requests; reopening the skill returns a fresh working URL. Its metadata/logs live in the shared temporary cache.
