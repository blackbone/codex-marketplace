...
input.on('line', async line => {
  ...
  try {
    if (message.method === 'initialize') return reply({ protocolVersion: versions.includes(message.params?.protocolVersion) ? message.params.protocolVersion : versions[0], capabilities: { tools: {} }, serverInfo: { name: 'docs', version }, instructions: 'Find documentation fragments with docs_search using the requested topic as query and absolute cwd. Return fragments with paths and line ranges. If needed, use docs_read for a matching range. Briefly report no results or unavailable search.' });
