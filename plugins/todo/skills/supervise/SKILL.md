...
2. `$todo:start` owns the rebind-to-current-chat flow. When this skill is invoked directly for first-time setup, create the heartbeat in the current chat. Do not infer that an existing binding belongs to the current chat because the repository binding does not store a chat ID.
...
4. After the host confirms creation, call `supervisor_bind` with the returned automation ID plus the exact name, prompt, recurrence, and confirmed status. The binding is mutable local `.todo` state and must not be committed.
5. When already configured and `actionRequired` is `resume` or `pause`, update that exact heartbeat with its complete stored definition, kind `heartbeat`, and the requested status. Only after the host confirms the update, persist the same status with `supervisor_bind`.
...
2. If `activeTasks.total` is zero, update this exact heartbeat to `PAUSED`, persist `PAUSED` through `supervisor_bind`, and stop. This is the single terminal detection run; once paused there must be no recurring idle runs. Do not delete the heartbeat.
...
After an operation creates or reactivates work, call `supervisor_get`. If it returns a configured binding with `actionRequired: "resume"`, update that exact heartbeat using its complete stored definition and `status: "ACTIVE"`; after host confirmation call `supervisor_bind` with the same definition and active status. Failure to resume supervision must be reported separately and must not be represented as failure of an otherwise successful task publication or runner operation.
