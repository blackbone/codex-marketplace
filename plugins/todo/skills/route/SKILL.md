...
6. After successful publication, call `supervisor_get`. If `actionRequired` is `"rebind"`, do not update the heartbeat; report that `$todo:start` must be invoked in the owner chat. If it is `"resume"`, update that exact heartbeat with its stored complete definition, explicitly pass its stored `targetThreadId`, and set `status: "ACTIVE"`; after host confirmation persist the same target and active definition with `supervisor_bind`. Never use the task-initiating chat as the update destination. Report a resume error separately without misrepresenting successful task publication. Leave tasks queued for background `codex exec` by default and do not implement them in the current thread.
