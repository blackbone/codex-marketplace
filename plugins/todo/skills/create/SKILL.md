...
1. Apply the purpose-based tooling exception above first; do not automatically create implementation tasks for tool setup. For the project portion, apply the injected Ponytail full execution contour. Inspect only enough repository context to make the task self-contained and resolve the real owner, flow, affected callers, existing reusable contract, smallest correct implementation, unnecessary alternatives, and exact minimal validation.
