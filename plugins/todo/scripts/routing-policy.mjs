...
export const WORKER_TOOLING_BOUNDARY = "The tooling and local verification exceptions do not expand a claimed worker's assigned task scope, repository access, permissions, or authority to create follow-up tasks. Perform tool setup or local verification only when required for the assigned task and already allowed by its restrictions; never use these exceptions to alter unrelated repositories, managed routing instructions, or .todo runtime state.";
...
- Route project mutations (create, modify, delete, rename, generate, format, commit, deploy, or other changes to project files or project-related state) through ToDo before performing them, even when the user does not mention ToDo. Apply the tooling and local verification exceptions above first, including their allowed generated outputs.
...
- Use interactive task execution only when the user explicitly requests it, or after a background attempt explicitly fails because it cannot proceed without a current-thread-only capability. In that case execute through \`task_run_start\` and \`task_run_finish\`. Direct local verification under the exception above is not task execution and does not acquire a claim.
